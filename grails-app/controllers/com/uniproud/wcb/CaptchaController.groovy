...
class CaptchaController {
    ...
    def index() {
        cache false
        def captcha = session[SimpleCaptchaService.CAPTCHA_SOLUTION_ATTR] ?: newCaptcha()
     ...
     */
    def validate(){
        Boolean captchaValid = simpleCaptchaService.validateCaptcha(params.param)
        if(captchaValid){
            render([status:'y'] as JSON)
        }else{
            render([status:'n',info:'识别码不正确！'] as JSON)
        }
    }
    /**
     * 手机效验
     */
    def checkmobile(){
        def count = Employee.countByMobile(params.mobile)
        if(count){
            render([status:'n',info: '手机号码已存在！'] as JSON)
        }else{
            render([status:'y'] as JSON)
        }
    }
...
//            session.setAttribute(SimpleCaptchaService.CAPTCHA_IMAGE_ATTR, captchaImage)
            session["${SimpleCaptchaService.CAPTCHA_SOLUTION_ATTR}"] = captchaSolution
            session["${SimpleCaptchaService.CAPTCHA_IMAGE_ATTR}"] = captchaImage
        } else {
