...
// }
//grails.config.locations = ["file:./grails-app/conf/db-config.properties"]
...
//grails.validateable.packages = ["com.uniproud.wcb"]
...
environments {
    development {
		...
		grails.config.locations = ["classpath:dev-jdbc.properties","dev-qx.properties"]
    }
    
    production {
        grails.logging.jul.usebridge = false
		grails.config.locations = ["classpath:prod-jdbc.properties","prod-qx.properties"]
//        grails.serverURL = "http://cloud.35crm.com"
        grails.serverURL = "http://cloud.xiaoshouwuyou.com"
    }
}
...
log4j = {
    ...
    environments {
        development {
            appender new org.apache.log4j.DailyRollingFileAppender(name:"dailyAppender",layout:pattern(conversionPattern: '[%-5p] %d{HH:mm:ss} %c{2} %m%n'),fileName:"log/log.log",datePattern:"'.'yyyy-MM-dd")
            console name:'stdout', layout:pattern(conversionPattern: '[%-5p] %d{HH:mm:ss} %c{2} %m%n')
            root{
                info  'stdout'
                additivity = false
            }
