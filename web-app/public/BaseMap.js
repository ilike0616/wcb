...
Ext.define("public.BaseMap",{
    ...
    initPathMap : function(div, dataObj) {
        ...
        Ext.Array.each(dataObj, function(data, index) {
            me.addMark(map,data.longtitude,data.latitude,data.name+":"+data.locusDate,data.location);
            //me.addMark(map,data.get("longtitude"),data.get("latitude"),data.get("name")+":"+data.get("locusDate"),data.get("location"));
        });
    },
    // 添加标记和文本
